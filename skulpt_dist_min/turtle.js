...
function generateTurtleModule(_target) {
    var _module              = {},
        ...
        _defaultSetup        = {
            ...
            width      : 860, // if set to 0 it will use the target width
        ...
        _max_distance_and_angles = 1000000,
        _poly,
        _creatingPoly = false,
        _frameRequest,
    ...
    function checkDistanceOrAngle(data) {
        if (Sk.builtin.checkNumber(data)){
            if (data > _max_distance_and_angles) {
                    throw new Sk.builtin.ValueError("Number is too large, don't over than 1,000,000")
                }
            }
    }
    ...
    (function(proto) {
        ...
        function animationFrame(delay) {
            ...
            return function(method) {
                ...
                _frameRequestTimeout = window.setTimeout(
                    method,
                    delay === 0 ? 0 : (OPTIMAL_FRAME_RATE || delay)
                );
        ...
        proto.reset = function() {
            ...
            this._frames         = [];
            this._frameCount     = 0;
            this._buffer         = _config.speedLevel;
            this._rate           = 0;
    ...
    function Turtle(shape) {
        ...
        if (!SHAPES.hasOwnProperty(this._shape)){
            throw Sk.builtin.ValueError("This shape not in default shape, please check again!")
        }
        this.reset();
    }
    ...
    (function(proto) {
        ...
        proto.$goto_$rw$ = proto.$setpos = proto.$setposition = function(x,y) {
            checkDistanceOrAngle(x);
            checkDistanceOrAngle(y);
            var coords = getCoordinates(x,y);
        ...
        proto.$pencolor = function(r,g,b,a) {
            if (r !== undefined) {
                this._color = createColor(baseColorMode,r,g,b,a);
        ...
        proto.$fillcolor = function(r,g,b,a) {
            if (r !== undefined) {
                this._fill = createColor(baseColorMode,r,g,b,a);
        ...
        proto.$color = function(color, fill, b, a) {
            if (color !== undefined) {
                if (fill === undefined || b !== undefined) {
                    this._color = createColor(baseColorMode, color, fill, b, a);
                    this._fill  = this._color;
                }
                else {
                    this._color = createColor(baseColorMode, color);
                    this._fill  = createColor(baseColorMode, fill);
                }
        ...
        proto.$get_poly = function(){
          if (_poly) {
              return Sk.ffi.remapToPy(_poly);
          }
        };

        proto.$dot = function(size, color, g, b, a) {
            checkDistanceOrAngle(size);
            pushUndo(this);
            ...
            color = (color !== undefined) ?
                createColor(baseColorMode, color, g, b, a) :
                this._color;
        ...
        proto.$clone = function() {
            ...
            newTurtleInstance.instance._colorMode = baseColorMode;
        ...
        proto.$resizemode = function (rmode) {
            if (rmode === undefined) {
                return Sk.builtin.str(_defaultSetup["rmode"])
            }
        };
        proto.$resizemode.minArgs = 0;
        proto.$resizemode.co_varnames = ["romode"];
        proto.$resizemode.default = [Sk.builtin.none.none$];

        proto.$pen = function (pen) {
            if (pen === undefined) {

            }
            return Sk.ffi.remapToPy(this._state)
        };
        proto.$pen.minArgs = 0;
        proto.$pen.co_varnames = ["pen"];
        proto.$pen.default = [Sk.builtin.none.none$];
    ...
    (function(proto) {
        ...
        proto.$bgcolor = function(color, g, b, a) {
            if (color !== undefined) {
                this._bgcolor = createColor(baseColorMode, color, g, b, a);
        ...
        proto.$screensize = function(canvwidth, canvheight, bg){
            if (canvwidth === undefined && canvheight === undefined && bg === undefined) {
                return  Sk.builtin.tuple([Sk.ffi.remapToPy(_config.width), Sk.ffi.remapToPy(_config.height)]);
            }

            if (canvwidth !== undefined) {
                _config.width = canvwidth;
                Sk.TurtleGraphics.width = canvwidth
            }

            if (canvheight !== undefined) {
                _config.height = canvheight;
                Sk.TurtleGraphics.height = canvheight;
            }

            this.$bgcolor(bg);

            _screenInstance = getScreen();
            _screenInstance.setUpWorld(-_config.width /2,-_config.height/2,_config.width/2,_config.height/2);

        };
        proto.$screensize.minArgs = 0;
        proto.$screensize.co_varnames = ["canvwidth", "canvheight", "bg"];
        proto.$screensize.$defaults = [Sk.builtin.none.none$, Sk.builtin.none.none$, Sk.builtin.none.none$];

        proto.$mode = function(mode){
            if (mode === undefined) {
                return Sk.builtin.str("logo")
            }
            if (mode !== "logo") {
                new Sk.builtin.NotImplementedError("Screen.mode() only support logo")
            }
            this.$reset()
        };
    ...
    function translate(turtle, startX, startY, dx, dy, beginPath, isCircle) {
        ...
        if (_creatingPoly) {
            _poly.push([startX + dx, startY + dy])
        }
            ...
            if(turtleColorMode === 255){//mode is 255
                for(i = 0; i < 3; i++) {
                    if(typeof color[i] === "number") {
                        ...
                        throw new Sk.builtin.ValueError("bad color sequence: (", + color + ")");
                    }
                }
    ...
    function addModuleMethod(klass, module, method, scopeGenerator) {
        ...
        wrapperFn = function() {
            ...
            if (args.length < minArgs || args.length > maxArgs) {
                ...
                console.log("displayName2222222222222222222222222:", displayName);
    ...
    function Vec2DWrapper($gbl, $loc) {

        // $loc.__init__ = new Sk.builtin.func(function (self, x, y) {
        //     console.log("x, y", x, y)
        // });
        $loc.__new__ = new Sk.builtin.func(function (self, x, y) {
            return Sk.builtin.tuple([x, y])
        });
        $loc.nb$add = function (x) {
            console.log("__add__ this:", this);
            console.log("__add__ x:", x);
        };
        $loc.nb$add.minArgs = 1;
        $loc.nb$add.co_varnames = ["Vec2D"];
        console.log("Vec2D.prototype:", Vec2D.prototype);
        for(var key in Vec2D.prototype) {
            if (/^\$[a-z_]+/.test(key)) {
                console.log("key:", key);
                addModuleMethod(Vec2D, $loc, key);
            }
        }
        Sk.abstr.setUpInheritance("Vec2D", Vec2D, Sk.builtin.tuple);
    }
    ...
    // _module.Vec2D = Sk.misceval.buildClass(_module, Vec2DWrapper, "Vec2D", []);
